...
"""
    ...
    prompt = f"""
You are an expert research analyst. Evaluate the following source excerpts and create a highly structured, informative overview of: "{topic}"

SOURCES:
{chr(10).join(used_blocks)}

Task Requirements:
1. EXECUTIVE SUMMARY: Write a compelling, concise paragraph summarizing the core subject.
2. KEY TAKEAWAYS: Extract the most crucial facts, statistics, or insights into an easy-to-read bulleted list.
3. IN-TEXT CITATIONS: You must back up your claims by referencing the source numbers in brackets (e.g., [1], [2]).
4. TONE: Professional, authoritative, and engaging.

Provide ONLY your final response. Do not include introductory filler or restate the sources block.
"""
